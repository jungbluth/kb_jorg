...
class JorgUtil:
    ...
    def check_input_assembly_for_minimum_coverage(self, task_params):
        ...
        if float(lines[1].split()[2]) >= float(task_params["min_coverage"]):
            log("The longest contig in the input assembly passes the input minimum coverage criteria. Setting Jorg coverage parameter to 75% of this value.")
            jorg_working_coverage = float(lines[1].split()[2]) * 0.75
